...
Named.prototype.all =
Named.prototype.items = function() {
  return this._list
}

Named.prototype.add = function(k, value) {
  id = value.id || value
  item = {
    id: id,
    value: value,
    name: this._useLower ? k.toLowerCase() : k
  }
  this[k] = value
  this._items[id] = item
...
Named.prototype.forEach = function(fn) {
  var i, ret
  for (var i in this._items) {
    ret = fn.call(this, this._items[i], i)
    if (ret === false) {
      break
    }
  }
...
Named.prototype.bind = function(model, attr, defination) {
  var self = this
  var raw_attr = '_' + attr
  // named ids are 2 bytes integer (-32768 to +32767)
  defination = defination || { type: Number, default: 0 }
  ...
  // when convert to json, always dump id to string
  model.prototype.toObject = function(onlySchema) {
    var ret = _toObject.apply(this, arguments)
    if (onlySchema) {
