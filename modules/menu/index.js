...
mesa.rest('/medias/:id/menu')
  ...
  .use(function* (next) {
    var media = yield Media.get(this.params.id)
    var wx = media && media.wx()
    this.assert(wx, 403, ERRORS.INVALID_APPKEY)
    yield* next
  })
  .get(function* () {
    ...
    // console.log("保存:"+ret);
    var menu = yield Menu.get(this.params.id);
  ...
  .post(function* saveMenu() {
    ...
    var menu = yield Menu.dump(this.params.id,this.req.body.menu)
    var result
    try{
      result = yield menu.sync()
    } catch(e) {
      this.assert(false, 500, ERRORS.WECHAT_API_FAIL)
    }
    
    this.body = { ok: true, result: result }
  })
